...
class AboutDialog {
  ...
  async show() {
    ...
    if (!this._metadataFetched) await this._fetchMetadata()
  }
  ...
  async _fetchMetadata() {
    try {
      const res = await fetch('./deployment-meta.json', { cache: 'no-store' })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      const data = await res.json()
      const hash = typeof data?.git_hash === 'string' ? data.git_hash.trim().slice(0, 8) : 'LOCAL'
      const date = typeof data?.date === 'number' && Number.isFinite(data.date)
        ? this._formatDate(data.date)
        : 'n/a'
      this._metadata = { gitHash: hash || 'LOCAL', deployed: date }
    } catch {
      this._metadata = { gitHash: 'LOCAL', deployed: 'n/a' }
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (logo) {
    ...
    logo.addEventListener('click', () => aboutDialog.show())
  }
})
